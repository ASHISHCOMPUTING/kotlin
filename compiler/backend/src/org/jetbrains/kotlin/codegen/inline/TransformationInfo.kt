...
class AnonymousObjectTransformationInfo internal constructor(
    ...
    override fun shouldRegenerate(sameModule: Boolean, isInlineLambdaContext: Boolean): Boolean =
        !alreadyRegenerated &&
                (!lambdasToInline.isEmpty() || !sameModule || capturedOuterRegenerated || needReification ||
                        capturesAnonymousObjectThatMustBeRegenerated ||
                        (
                                //inline lambda could have only stateless lambdas
                                //TODO: process stateless local functions and lambdas from inline function
                                isStaticOrigin && isInlineLambdaContext)
                        )
